...
# Link em destaque (Dados completos)
LOOKER_DADOS_COMPLETOS_URL = 'https://lookerstudio.google.com/reporting/924bcb55-745f-49c1-aeab-72dc5edc72e5'
...
def gerar_docx_certidao_internal(tipo, numero, data, consultor, motivo, chamado="", hora=""):
    try:
        ...
        runner.bold = True
        head_p.add_run(
            "Rua Ouro Preto, N° 1564 - Bairro Santo Agostinho - CEP 30170-041 - Belo Horizonte - MG\n"
        ...
        if tipo == 'Geral':
            txt = (
                "Para fins de cumprimento dos artigos 13 e 14 da Resolução nº 780/2014 do Tribunal de Justiça do Estado de Minas Gerais, "
                f"informamos que em {data} houve indisponibilidade do portal JPe, superior a uma hora, {hora}, "
                "que impossibilitou o peticionamento eletrônico de recursos em processos que já tramitavam no sistema."
            )
            corpo.add_run(txt)
        else:
            corpo.add_run(
                f"Informamos que no dia {data}, houve indisponibilidade específica do sistema para o peticionamento do processo nº {numero}.\n\n"
            )
            corpo.add_run(
                f"O Chamado de número {chamado if chamado else '_____'}, foi aberto e encaminhado à DIRTEC (Diretoria Executiva de Tecnologia da Informação e Comunicação).\n\n"
            )
            if tipo == 'Física':
                corpo.add_run(
                    "Diante da indisponibilidade específica, não havendo um prazo para solução do problema, a Primeira Vice-Presidência recomenda o ingresso dos autos físicos, "
                    "nos termos do § 2º, do artigo 14º, da Resolução nº 780/2014, do Tribunal de Justiça do Estado de Minas Gerais.\n\n"
                )
            else:
                corpo.add_run(
                    "Informamos a indisponibilidade para fins de restituição de prazo ou providências que V.Exa julgar necessárias, nos termos da legislação vigente.\n\n"
                )
...
def save_state():
    try:
        ...
        state_to_save = {
            ...
            # --- NOVO: Operacional ---
            'operacional_logs': st.session_state.get('operacional_logs', []),
            'operacional_drafts': st.session_state.get('operacional_drafts', {}),
        }
...
# --- FUNCAO CRITICA: SINCRONIZAR COM BD NO REFRESH ---
def sync_state_from_db():
    try:
        # So sincroniza quando nao estiver em modo de registro (active_view == None).
        db_data = load_state_from_db()
        if not db_data:
            return

        # Chaves criticas que precisam refletir entre usuarios
        keys = [
            'status_texto', 'bastao_queue', 'skip_flags', 'bastao_counts',
...
# --- Operacional (NOVO) ---
def _append_operacional_log(consultor: str, tipo: str, texto: str):
    now_br = get_brazil_time()
    st.session_state.operacional_logs.append({
        'timestamp': now_br,
        'consultor': consultor,
        'tipo': tipo,
...
def send_operacional_to_chat(consultor: str, tipo: str, texto: str) -> bool:
    texto = (texto or '').strip()
    if not texto:
        return False
    ...
    # Usa o mesmo canal do registro (mantém compatibilidade com secrets já existentes)
    send_to_chat('registro', msg)
    return True


def handle_operacional_submission(tipo: str, draft_key: str):
    ...
    texto = st.session_state.get(draft_key, '')
    if not (texto or '').strip():
        st.warning('Digite o texto do operacional antes de enviar.')
        return False

    ok = False
    try:
        ok = send_operacional_to_chat(consultor, tipo, texto)
    except Exception:
        ok = False

    if ok:
        _append_operacional_log(consultor, tipo, texto)
        # limpa draft
        st.session_state[draft_key] = ''
        st.session_state.operacional_drafts[tipo] = ''
        save_state()
...
def send_chat_notification_internal(consultor, status):
    if CHAT_WEBHOOK_BASTAO and status == 'Bastão':
        msg = (
            f"🎉 **BASTÃO GIRADO!** 🎉 \n\n"
            f"- **Novo(a) Responsável:** {consultor}\n"
            f"- **Acesse o Painel:** {APP_URL_CLOUD}"
        )
    ...
    if now.hour >= 23 and now.date() == last_run.date():
        ...
        if any(s != 'Indisponível' for s in st.session_state.status_texto.values()) or st.session_state.bastao_queue:
            ...
            for n in CONSULTORES:
                st.session_state[f'check_{n}'] = False
            save_state()
...
def init_session_state():
    ...
    if 'report_last_run_date' in st.session_state and isinstance(st.session_state['report_last_run_date'], str):
        try:
            st.session_state['report_last_run_date'] = datetime.fromisoformat(st.session_state['report_last_run_date'])
        except Exception:
    ...
    defaults = {
        ...
        'bastao_queue': [],
        'skip_flags': {},
        'current_status_starts': {nome: now for nome in CONSULTORES},
        ...
        # --- NOVO: Operacional ---
        'operacional_logs': [],
        'operacional_drafts': {'Chat': '', 'HP': '', 'Atendimentos': ''},
        'op_chat_text': '',
        'op_hp_text': '',
        'op_atend_text': '',
    }
    ...
    # garante dicionário
    if not isinstance(st.session_state.get('operacional_drafts'), dict):
        st.session_state.operacional_drafts = {'Chat': '', 'HP': '', 'Atendimentos': ''}
    ...
    for nome in CONSULTORES:
        ...
        if nome not in st.session_state.current_status_starts:
            st.session_state.current_status_starts[nome] = now
...
def reset_day_state():
    ...
    # --- NOVO: limpa operacional no reset diário ---
    st.session_state.operacional_logs = []
    st.session_state.operacional_drafts = {'Chat': '', 'HP': '', 'Atendimentos': ''}
    st.session_state.op_chat_text = ''
    st.session_state.op_hp_text = ''
    st.session_state.op_atend_text = ''
...
def toggle_queue(consultor):
    ...
    if now_hour >= 20 or now_hour < 6:
        st.toast("💤 Fora do expediente!", icon="🌙")
        return False
...
def toggle_skip():
    ...
    # Pular mantém o lugar na fila (não move pro final)
    save_state()
    st.rerun()
...
with c_topo_esq:
    img = get_img_as_base64(PUG2026_FILENAME)
    src = f"data:image/png;base64,{img}" if img else GIF_BASTAO_HOLDER
    st.markdown(
        f"""
<div style="display: flex; align-items: center; gap: 15px;">
  <h1 style="margin: 0; padding: 0; font-size: 2.2rem; color: #FFD700; text-shadow: 1px 1px 2px #B8860B;">Controle Bastão Cesupe 2026 {BASTAO_EMOJI}</h1>
  <img src="{src}" style="width: 120px; height: 120px; border-radius: 50%; border: 3px solid #FFD700; object-fit: cover;">
</div>
""",
        unsafe_allow_html=True,
    )
...
with c_topo_dir:
    ...
    with c_sub1:
        novo_responsavel = st.selectbox(
            "Assumir Bastão (Rápido)",
            options=["Selecione"] + CONSULTORES,
...
if st.session_state.active_view is None:
    ...
    # Sincroniza estado do banco (para múltiplos usuários verem as mudanças)
    sync_state_from_db()
else:
...
with col_principal:
    ...
    if responsavel:
        st.markdown(
            f"""
<div style="background: linear-gradient(135deg, #FFF8DC 0%, #FFFFFF 100%); border: 3px solid #FFD700; padding: 25px; border-radius: 15px; display: flex; align-items: center; box-shadow: 0 4px 15px rgba(255, 215, 0, 0.3); margin-bottom: 20px;">
  <div style="flex-shrink: 0; margin-right: 25px;">
    <img src="{GIF_BASTAO_HOLDER}" style="width: 90px; height: 90px; border-radius: 50%; object-fit: cover; border: 2px solid #FFD700;">
  </div>
  <div>
    ...
    <span style="font-size: 42px; font-weight: 800; color: #000080; line-height: 1.1;">{responsavel}</span>
  </div>
</div>
""",
            unsafe_allow_html=True,
    ...
    if st.session_state.active_view == 'menu_atividades':
        with st.container(border=True):
            at_t = st.multiselect("Tipo:", OPCOES_ATIVIDADES_STATUS)
            at_e = st.text_input("Detalhe:")
            if st.button("Confirmar Atividade"):
                st.session_state.active_view = None
                update_status(f"Atividade: {', '.join(at_t)} - {at_e}")
    ...
    # --- NOVO BOTÃO / VIEW: Operacional ---
    c_tool7.button("🧾 Operacional", help="Registro Operacional (Chat / HP / Atendimentos)", use_container_width=True, on_click=toggle_view, args=("operacional",))

    if st.session_state.active_view == "operacional":
        with st.container(border=True):
            ...
            with t_chat:
                st.text_area("Operacional - Chat", key='op_chat_text', height=180)
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("📨 Enviar (Chat)", type='primary', use_container_width=True, key='btn_op_chat'):
                        ok = handle_operacional_submission('Chat', 'op_chat_text')
                        if ok:
                            st.success('Operacional (Chat) enviado!')
                            st.session_state.active_view = None
                            st.rerun()
                with c2:
                    if st.button("❌ Cancelar", use_container_width=True, key='btn_op_chat_cancel'):
                        st.session_state.active_view = None
                        st.rerun()

            with t_hp:
                st.text_area("Operacional - HP", key='op_hp_text', height=180)
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("📨 Enviar (HP)", type='primary', use_container_width=True, key='btn_op_hp'):
                        ok = handle_operacional_submission('HP', 'op_hp_text')
                        if ok:
                            st.success('Operacional (HP) enviado!')
                            st.session_state.active_view = None
                            st.rerun()
                with c2:
                    if st.button("❌ Cancelar", use_container_width=True, key='btn_op_hp_cancel'):
                        st.session_state.active_view = None
                        st.rerun()

            with t_atend:
                st.text_area("Operacional - Atendimentos", key='op_atend_text', height=180)
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("📨 Enviar (Atendimentos)", type='primary', use_container_width=True, key='btn_op_atend'):
                        ok = handle_operacional_submission('Atendimentos', 'op_atend_text')
                        if ok:
                            st.success('Operacional (Atendimentos) enviado!')
                            st.session_state.active_view = None
                            st.rerun()
                with c2:
                    if st.button("❌ Cancelar", use_container_width=True, key='btn_op_atend_cancel'):
                        st.session_state.active_view = None
                        st.rerun()

            if st.session_state.get('operacional_logs'):
                st.markdown('---')
                st.subheader('📌 Últimos operacionais (Hoje)')
                logs = st.session_state.operacional_logs[-10:][::-1]
                df = pd.DataFrame([
                    {
                        'Hora': x['timestamp'].strftime('%H:%M'),
                        'Consultor': x['consultor'],
                        'Tipo': x['tipo'],
                        'Resumo': (x.get('texto') or '').strip()[:120] + ('…' if len((x.get('texto') or '').strip()) > 120 else ''),
                    }
                    for x in logs
                ])
    ...
    if st.session_state.active_view == "checklist":
        with st.container(border=True):
            ...
            data_eproc = st.date_input("Data:", value=get_brazil_time().date())
            camara_eproc = st.selectbox("Câmara:", CAMARAS_OPCOES)
            if st.button("Gerar HTML"):
                st.write("Checklist registrado no chat!")
    ...
    if st.session_state.active_view == "hextras":
        with st.container(border=True):
            ...
            h_in = st.time_input("Início:")
            t_ex = st.text_input("Tempo Total:")
            mot = st.text_input("Motivo:")
            if st.button("Registrar", type='primary', use_container_width=True):
                send_horas_extras_to_chat(st.session_state.consultor_selectbox, d_ex, h_in, t_ex, mot)
    ...
    if st.session_state.active_view == "erro_novidade":
        with st.container(border=True):
            ...
            rel = st.text_area("Relato:")
            res = st.text_area("Resultado:")
            if st.button("Enviar", type='primary', use_container_width=True):
                handle_erro_novidade_submission(st.session_state.consultor_selectbox, tit, obj, rel, res)
                st.success("Enviado!")
                st.session_state.active_view = None
                st.rerun()
            if st.button("❌ Cancelar Registro"):
    ...
    if st.session_state.active_view == "certidao":
        with st.container(border=True):
            ...
            if tipo_cert == "Geral":
                ...
                c_proc = col_c2.text_input("Nº Processo:")
                c_motivo = st.text_area("Motivo / Erro:")

            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                if st.button("📄 Gerar Modelo Word", use_container_width=True):
                    st.session_state.word_buffer = gerar_docx_certidao_internal(
                        tipo_cert,
                        c_proc,
                        c_data.strftime("%d/%m/%Y"),
                        c_cons,
                        c_motivo,
                        c_chamado if tipo_cert != 'Geral' else '',
                        c_hora,
                    )
            ...
            with col_btn2:
                if st.button("💾 Salvar Registro", type="primary", use_container_width=True):
                    if verificar_duplicidade_certidao(tipo_cert, c_proc, c_data, c_hora):
                        st.session_state.aviso_duplicidade = True
                    else:
                        payload = {
                            "tipo": tipo_cert,
                            "data_evento": c_data.isoformat(),
                            "consultor": c_cons,
                            "n_chamado": c_chamado if tipo_cert != 'Geral' else '',
                            "n_processo": c_proc.strip().rstrip('.'),
                            "motivo": c_motivo,
                            "hora_periodo": c_hora,
                        }
            ...
            if st.button("❌ Cancelar Registro", key="cancel_cert"):
                st.session_state.active_view = None
                st.rerun()
    ...
    def _render_section_detalhada(titulo, icon, lista_tuplas, badge_color, keyword_removal):
        ...
        if not lista_tuplas:
            ...
            for nome, desc in sorted(lista_tuplas, key=lambda x: x[0]):
                ...
                col_check.checkbox(
                    ' ',
                    key=f'chk_{titulo}_{nome}',
                    value=True,
                    on_change=leave_specific_status,
                    args=(nome, keyword_removal),
                    label_visibility='collapsed',
                )
                col_nome.markdown(
