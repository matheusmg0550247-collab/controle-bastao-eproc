...
# OTIMIZAÇÃO: TTL de 1 hora na conexão para evitar queda por inatividade
@st.cache_resource(ttl=3600)
def get_supabase():
    try: 
        return create_client(st.secrets["supabase"]["url"], st.secrets["supabase"]["key"])
    except Exception as e:
        # Limpa cache para tentar reconectar na proxima execução se falhar
...
# ============================================
# 3. REPOSITÓRIO (COM CACHE INTELIGENTE 2s)
# ============================================
...
def memory_sweeper():
    ...
    # Executa a limpeza leve a cada 5 minutos (300 segundos)
    if time.time() - st.session_state.last_cleanup > 300:
        st.session_state.word_buffer = None 
        gc.collect()
        st.session_state.last_cleanup = time.time()
        
    # Limpeza PESADA a cada 4 horas
...
def auto_manage_time():
    ...
    if now.hour >= 23 and now.date() == last_run_dt.date(): reset_day_state(); save_state()
    elif now.date() > last_run_dt.date(): reset_day_state(); save_state()
            ...
            if db_data:
                ...
                if 'report_last_run_date' in db_data and isinstance(db_data['report_last_run_date'], str):
                    try:
                        ...
                        db_data['report_last_run_date'] = datetime.min
